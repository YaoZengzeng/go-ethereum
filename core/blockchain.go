...
// insert插入一个新的head block到当前的block chain，该方法假设这个block是一个真正的head
// 它同时会重置header以及head fast sync block，如果它们更老或者位于不同的side chain
//
...
func (bc *BlockChain) insert(block *types.Block) {
	...
	// 将block加入canonical chain number scheme并且标记为head
	...
	// 如果block比我们的head更好或者在不同的chain，强制更新heads
	if updateHeads {
...
// WriteBlockWithState将block以及所有相关的state写入数据库中
func (bc *BlockChain) WriteBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB) (status WriteStatus, err error) {
	...
	if reorg {
		...
		// 如果block的parent不是head block，则对chain进行重组
		...
		// 写入潜在的元数据用于transaction/receipt的查询以及preimages
		rawdb.WriteTxLookupEntries(batch, block)
...
// event about them
// reorgs根据两个block，一个old chain以及一个new chain并且会重构blocks
...
// PostChainEvents遍历所有插入chain产生的events并且将它们发送给event feed
